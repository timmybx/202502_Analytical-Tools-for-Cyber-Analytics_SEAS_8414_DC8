...
if not submitted:
    ...
    with st.status("Executing SOAR playbook...", expanded=True) as status:
        ...
        st.write(
            f"▶️ **Step 2: Verdict Interpretation** - Model predicts **{verdict}**."
        )
        time.sleep(1)

        if is_malicious:
            st.write(
                f"▶️ **Step 3: Prescriptive Analytics** - Engaging **{genai_provider}** for action plan."
            )
            try:
                ...
                prescription = None
                status.update(
                    label="🚨 Error during GenAI prescription!", state="error"
                )
        else:
            prescription = None
            status.update(
                label="✅ Analysis Complete. No threat found.",
                state="complete",
                expanded=False,
            )
